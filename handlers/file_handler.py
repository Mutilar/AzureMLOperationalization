...
def add_notebook_callback(params, notebook, run_id, postexec=None, preexec=None):
    ...
    notebook_obj.inject_code(
        ...
        code=[
            ...
            nh.TAB + "username=os.environ[\"SP_USERNAME\"],", # + params["azure_resources"]["service_principal"]["username"] + "\",",
            nh.TAB + "tenant=os.environ[\"SP_TENANT\"],",     # + params["azure_resources"]["service_principal"]["tenant"]   + "\",",
            nh.TAB + "password=os.environ[\"SP_PASSWORD\"],", # + params["azure_resources"]["service_principal"]["password"] + "\",",
